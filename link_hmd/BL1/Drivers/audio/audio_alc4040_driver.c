...
static struct audio_alc4040_data g_alc4040_data;
...
int audio_alc4040_audio_path_set(enum __audio_4040_path new_path)
{
	...
	case AUDIO_4040_USB_PATH:
		if (data->path_switch_control(new_path) == 0) {
			audio_4040_info("%s: audio path setup as %s\r\n",
...
int audio_alc4040_driver_init()
{
	...
	if (alc4040_data->is_initialized == 1) {
		audio_4040_info("%s: driver already initialized, exit\r\n",
								__func__);
		return ret;
	}
	...
	audio_4040_info("%s: driver initial successfully\n", __func__);
