...
static int audio_alc5665_power_control(unsigned char onoff)
{
	...
	if (onoff)
		...
		data->path = AUDIO_5665_UNSET_PATH;
...
int alt5665_audio_path_set(enum __audio_5665_path new_path)
{
	...
	case AUDIO_5665_HDMI_PATH:
		audio_5665_info("%s: audio path setup as HDMI\r\n", __func__);
	...
	case AUDIO_5665_USB_PATH:
		audio_5665_info("%s: audio path setup as USB\r\n", __func__);
	...
	case AUDIO_5665_LOOPBACK_PATH:
		audio_5665_info("%s: audio path setup as LOOPBACK\r\n",
								__func__);
	...
	case AUDIO_5665_OUTPUT_TO_AJ:
		audio_5665_info("%s: audio output setup as AJ\r\n",
								__func__);
	...
	case AUDIO_5665_OUTPUT_TO_EAR:
		audio_5665_info("%s: audio output setup as EAR\r\n",
								__func__);
	...
	case AUDIO_5665_OUTPUT_TO_BOTH:
		audio_5665_info("%s: audio output setup as BOTH\r\n",
								__func__);
		if (alt5665_regs_set(data->lout_unmute_list,
					data->lout_unmute_list_count) != 0) {
...
int audio_alc5665_driver_init(void)
{
	...
	if (alc5665_data_p->is_initialized == 1) {
		audio_5665_info("%s: driver already initialized, exit\r\n",
								__func__);
		return ret;
	}
	...
	audio_alc5665_power_control(1);

	if (audio_alc5665_check_pid_vid(alc5665_data_p) != 0) {
		audio_alc5665_power_control(0);
	...
	alc5665_data_p->init_reg_list_count = sizeof(alc5665_evm_init_list) /
	...
	audio_5665_info("%s: driver initial successfully\n", __func__);
	return ret;
}
