...
void DFU_move_img(USBD_HandleTypeDef *pdev, USBD_SetupReqTypedef *req);
...
USBD_DFU_HandleTypeDef usbd_dfu_classdata={0};
                               ...
                               uint8_t cfgidx)
{
 ...
 /* Allocate Audio structure */
  pdev->pClassData = &usbd_dfu_classdata; ////USBD_malloc(sizeof (USBD_DFU_HandleTypeDef));
...
/**
  ...
  * @retval status
  */
extern uint32_t g_FOTAMoveFlag;
static uint8_t  USBD_DFU_Setup (USBD_HandleTypeDef *pdev, 
                                USBD_SetupReqTypedef *req)
{
  uint8_t *pbuf = 0;
...
void dfu_move_cb(USBD_HandleTypeDef *pdev,int state){
       USBD_DFU_HandleTypeDef   *hdfu=NULL;
       hdfu = (USBD_DFU_HandleTypeDef*) pdev->pClassData;
       printf("%s state=%d hdfu=%p \n",__func__,state,hdfu);

       if(state==1){
               hdfu->data_ptr = DFU_ERROR_NONE; ////successful
       }else if(state>1){
               hdfu->data_ptr = state; ////failed
       }else if(state==0){
               hdfu->data_ptr = DFU_ERROR_UNKNOWN;
	   }
       hdfu->dev_state = DFU_STATE_MOVEIMG_DONE;
}

/**
  * @brief  DFU_move_img
  *         Handles the DFU move img request.
  * @param  pdev: device instance
  * @param  req: pointer to the request structure
  * @retval None
  */
void DFU_move_img(USBD_HandleTypeDef *pdev, USBD_SetupReqTypedef *req)
{
    ...
    if ((hdfu->dev_state == DFU_STATE_IDLE) ){
		printf("%s state=%d value=%d ok,can move img!\n",__func__,hdfu->dev_state,req->wValue);
    }
    else{
		printf("%s state=%d value=%d  the state is right?? \n",__func__,hdfu->dev_state,req->wValue);
    }
// typedef void dfu_move_img_callback(USBD_HandleTypeDef *pdev,int state);
// move api:  void  fota_move_image(uint16_t value,dfu_move_img_callback cb,USBD_HandleTypeDef *cb_arg);
	fota_move_image(0,dfu_move_cb,pdev);
...
static void DFU_GetStatus(USBD_HandleTypeDef *pdev)
{
 ...
 hdfu = (USBD_DFU_HandleTypeDef*) pdev->pClassData;
  ...
  case   DFU_STATE_MOVEIMG_DONE :
	/* call USBD_CtlSendData to send	'the move state'  to usb host */
	hdfu->dev_state = DFU_STATE_MOVEIMG_DONE;

	hdfu->dev_status[0] = (uint8_t)hdfu->data_ptr;
	hdfu->dev_status[1] = 0;
	hdfu->dev_status[2] = 0;
	hdfu->dev_status[3] = 0;
	hdfu->dev_status[4] = hdfu->dev_state;
...
static void DFU_Abort(USBD_HandleTypeDef *pdev)
{
 ...
 hdfu = (USBD_DFU_HandleTypeDef*) pdev->pClassData;
  ...
  if (hdfu->dev_state == DFU_STATE_IDLE || hdfu->dev_state == DFU_STATE_DNLOAD_SYNC
      || hdfu->dev_state == DFU_STATE_DNLOAD_IDLE || hdfu->dev_state == DFU_STATE_MANIFEST_SYNC
        || hdfu->dev_state == DFU_STATE_UPLOAD_IDLE || hdfu->dev_state == DFU_STATE_MOVEIMG_DONE)
  {
    hdfu->dev_state = DFU_STATE_IDLE;
    hdfu->dev_status[0] = DFU_ERROR_NONE;
    hdfu->dev_status[1] = 0;
