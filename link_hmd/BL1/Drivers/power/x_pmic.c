    ...
    switch(item){
        case V_BOOST_5V_EN:
            if(XA0n == pcb_id){
                ...
                ret = ioexp_gpio_get_value(IOEXP_REG_DIR,
                        IOEXP_I_V_BOOST_5V_EN);
            }
            break;
        case V_USB30_5V_EN:
            ret = ioexp_gpio_get_value(IOEXP_REG_DIR, IOEXP_V_USB30_5V_EN);
            break;
        case V_DP_EN:
            if(XA0n == pcb_id){
                ...
                ret = ioexp_gpio_get_value(IOEXP_REG_DIR, IOEXP_II_V_DP_PS_EN);
            }
            break;
			/*
        case V_HDMI_EN:
        ...
        case V_VDDI_L_EN:
            if(XA0n == pcb_id){
                ...
                ret = ioexp_gpio_get_value(IOEXP_REG_DIR, IOEXP_I_V_VDDI_L_EN);
            }
            break;
        case V_VDDI_R_EN:
            if(XA0n == pcb_id){
                ...
                ret = ioexp_gpio_get_value(IOEXP_REG_DIR, IOEXP_I_V_VDDI_R_EN);
            }
            break;
        case V_VCI_3V3_L:
        ...
        case V_PX_1V1_EN:
            if((XB02 == pcb_id)||(XC01 == pcb_id) || (XC02 == pcb_id)){
                ret = ioexp_gpio_get_value(IOEXP_REG_DIR, IOEXP_II_V_PX_1V1_EN
);
            }
            break;
        case V_PX_1V8_EN:
            if((XB02 == pcb_id)||(XC01 == pcb_id) || (XC02 == pcb_id)){
                ret = ioexp_gpio_get_value(IOEXP_REG_DIR, IOEXP_II_V_PX_1V8_EN
);
            }
            break;
        default:
...
int bsp_pmic_get_pg_status(void)
{
    uint8_t state = 0;
    PmicDrvTypeDef *handle = &PMIC_Handle;
	PmicFunTable_t *driver = (PmicFunTable_t*)((PmicDrvTypeDef*)handle)->pVTable;
...
int bsp_pmic_get_charger_status(PmicDrvTypeDef * handle, uint8_t *state)
{
	PmicFunTable_t *driver = (PmicFunTable_t*)((PmicDrvTypeDef*)handle)->pVTable;
	CHARGER_DRV_t *ch_driver = (CHARGER_DRV_t*)(driver->charger);
	...
	if (ch_driver->Get_chargingState == NULL) return -1;
	ch_driver->Get_chargingState(handle, state);
...
int bsp_pmic_get_battery_volt(PmicDrvTypeDef * handle, uint32_t* volt)
{
    int ret = 0;
    PmicFunTable_t *driver = (PmicFunTable_t*)((PmicDrvTypeDef*)handle)->pVTable;
    ...
    ret |= ga_driver->Get_Voltage(handle, volt);

    return ret;
}

int bsp_pmic_get_battery_soc(PmicDrvTypeDef * handle, int* soc)
{
    int ret = 0;
    PmicFunTable_t *driver = (PmicFunTable_t*)((PmicDrvTypeDef*)handle)->pVTable;
    GAUGE_DRV_t * ga_driver = (GAUGE_DRV_t*)(driver->gauge);
    ...
    ret |= ga_driver->Get_Soc(handle, soc);

    return ret;
}


int bsp_pmic_get_battery_temp(PmicDrvTypeDef * handle, int* temp)
{
    int ret = 0;
    PmicFunTable_t *driver = (PmicFunTable_t*)((PmicDrvTypeDef*)handle)->pVTable;
    GAUGE_DRV_t * ga_driver = (GAUGE_DRV_t*)(driver->gauge);
    ...
    ret |= ga_driver->Get_Temperature(handle, temp);

    return ret;
}


int bsp_pmic_get_battery_curr(PmicDrvTypeDef * handle, int* curr)
{
    int ret = 0;
    PmicFunTable_t *driver = (PmicFunTable_t*)((PmicDrvTypeDef*)handle)->pVTable;
    GAUGE_DRV_t * ga_driver = (GAUGE_DRV_t*)(driver->gauge);
    ...
    ret |= ga_driver->Get_Current(handle, 0, curr);
