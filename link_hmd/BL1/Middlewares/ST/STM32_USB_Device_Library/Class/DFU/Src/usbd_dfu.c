...
static void DFU_move_img(USBD_HandleTypeDef *pdev, USBD_SetupReqTypedef *req);
...
void dfu_move_cb(USBD_HandleTypeDef *pdev,uint8_t state){
       ...
       if(state){
               ...
               hdfu->data_ptr = DFU_ERROR_UNKNOWN; ////failed
       }
...
void DFU_move_img(USBD_HandleTypeDef *pdev, USBD_SetupReqTypedef *req)
{
    ...
    if ((hdfu->dev_state == DFU_STATE_IDLE) ){
		...
		printf("%s state=%d value=%d err!!! the state is wrong!!! can't move? \n",__func__,hdfu->dev_state,req->wValue);
    }
// typedef void dfu_move_img_callback(USBD_HandleTypeDef *pdev,int state);
// move api:  void  real_move_img(uint16_t value,dfu_move_img_callback cb,USBD_HandleTypeDef *cb_arg);
// real_move_img(req->wValue,dfu_move_cb,pdev);
}
