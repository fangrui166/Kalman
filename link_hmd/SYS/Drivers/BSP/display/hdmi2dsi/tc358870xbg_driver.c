...
#define CHECK_GAMMA_SETTING             (1) //it's depend on g_disp_fps
...
typedef struct {
    ...
    const reg_item *startup_p1_tbl;
    uint32_t startup_p2_tbl_size;
    const reg_item *startup_p2_tbl;
    uint32_t colorbar_p1_tbl_size;
    const reg_item *colorbar_p1_tbl;
    uint32_t colorbar_p2_tbl_size;
    const reg_item *colorbar_p2_tbl;
} h2d_tbl_t;

#include "ocean_video_timing.h"
#include "ocean_note_video_timing.h"
#include "m10_video_timing.h"

static const h2d_tbl_t *cur_h2d_tbl = &ocean_h2d_tbl;

#if ENABLE_GET_REGS_FROM_PARTION
static h2d_tbl_t partion_h2d_tbl;
...
static void readFromDSI(int dsi_if)
{
    ...
    if (((value>>16)&0xFF) == 0x1C) {//long packet.
        ...
        disp_err("check dsi%d: unknow packet type.\n", dsi_if);
    }
}
    ...
    if (((value>>16)&0xFF) == 0x1C) {//long packet.
        ...
        for (j = 0; j < cnt; j++) {
            ...
            disp_debug("Received data 0x%02x, 0x%02x, 0x%02x, 0x%02x.\n", ((uint8_t*)&value)[0],
                                                                ...
                                                                ((uint8_t*)&value)[2],
                                                                ((uint8_t*)&value)[3]);
            if (cur_h2d_tbl->fps > 50 && cur_h2d_tbl->fps < 70) //reinit gamma for 60fps
...
static int h2d_sync_int_check(void)
{
    ...
    if (s_status & BIT(9)) {
        ...
        disp_info("other interrrupt source enanbled?\n");
    #if INT_STATUS_CHECK_ENABLE
        int_status_check();
    #endif
    }
...
extern void htc_3dof_update_display_status(uint8_t value);
...
static int sync_event_handle(void)
{
    ...
    if (b_status & BIT(7)) {
		...
		// dp connected
#if 0
		SendLedEventToDeal(LED_EVENT_DISCONNECTED | LED_EVENT_OFF);
		SendLedEventToDeal(LED_EVENT_CONNECTING | LED_EVENT_OFF); // equal disconnected off
		SendLedEventToDeal(LED_EVENT_CONNECTED | LED_EVENT_ON); // default on
#else
		pwrmgr_system_send_message(PM_SYS_CMD_DP_SYNC_STATE, DP_SYNC_DET);
#endif
        htc_3dof_update_display_status(1);
        return 1;
    } else {
        disp_debug("synchronization is lost.\n"); // fixme. Maybe need to start a timer to check. or just reinitialize.
		// dp disconnected
#if 0
		SendLedEventToDeal(LED_EVENT_DISCONNECTED | LED_EVENT_ON);
#else
		pwrmgr_system_send_message(PM_SYS_CMD_DP_SYNC_STATE, DP_SYNC_LOST);
#endif
        htc_3dof_update_display_status(0);
...
void l_gi_flm_1_int_handle(void)
{
	...
	float fps = cur_h2d_tbl->fps;
	float actual_fps;

	if (!l_gi_flm_1_int_cnt) {
		tick_start = HAL_GetTick();  // start at l_gi_flm_1_int_cnt = 1
	}
	l_gi_flm_1_int_cnt++;
	if (HAL_GetTick() - tick_start > 1000) { // check per 1s
		l_gi_flm_1_int_cnt--;
		actual_fps = (float)l_gi_flm_1_int_cnt / (HAL_GetTick() - tick_start) * 1000;
		if (actual_fps < fps - tolerance || actual_fps > fps + tolerance) { // 1.power off  2.power on
			disp_err("monitor L_GI pin per second error, fps=%f, actual_fps=%f.\n", fps, actual_fps);
			// reset display
#if 0
			BaseType_t need_resched = pdFALSE;
...
#else
    disp_info("gi monitor is disabled.");
#endif
}
...
void r_gi_flm_1_int_handle(void)
{
	...
	if (!r_gi_flm_1_int_cnt) {
		tick_start = HAL_GetTick();  // start at r_gi_flm_1_int_cnt = 1
	}
	r_gi_flm_1_int_cnt++;
	htc_3dof_update_vsync_status(0);
	if (HAL_GetTick() - tick_start > 1000) { // check per 1s
		r_gi_flm_1_int_cnt--;
		actual_fps = (float)r_gi_flm_1_int_cnt / (HAL_GetTick() - tick_start) * 1000;
		if (actual_fps < fps - tolerance || actual_fps > fps + tolerance) { // 1.power off  2.power on
			disp_err("monitor R_GI pin per second error, fps=%f, actual_fps=%f.\n", fps, actual_fps);
			// reset display
#if 0
			BaseType_t need_resched = pdFALSE;
			if (h2d_task_handle && gi_flm_err_rst_en) {
...
void panel_error_monitor(uint16_t gpio_pin)
{
	...
	else if (((pcb_id != XC01) && (GPIO_XB_R_GI_FLM_1_IO == gpio_pin)) ||
        ((pcb_id == XC01) && (GPIO_XC_R_GI_FLM_1_IO == gpio_pin)) ||
        ((pcb_id == XC02) && (GPIO_XC_R_GI_FLM_1_IO == gpio_pin)))
...
void disp_read_gamma(void)
{
    ...
    check_gamma_settings();
#else
    disp_warning("read gamma is disabled.\n");
#endif
}
...
#if OS_ENABLE
	while (HAL_GetTick() - disp_power_off_last_tick < 2000) osDelay(500);	// no delay, CES: power off -> delay 2s -> power on
#else
	while (HAL_GetTick() - disp_power_off_last_tick < 2000);	// no delay, CES: power off -> delay 2s -> power on
#endif
...
static void h2d_gpio_config(void)
{
    ...
    if(XA0n == pcb_id){
		...
		if (XB02 == pcb_id) {
			...
			GPIO_InitStruct.Pin = GPIO_XC_R_GI_FLM_1_IO;
			HAL_GPIO_Init(GPIO_XC_PORT_R_GI_FLM_1_IO, &GPIO_InitStruct);
		}
    }
...
static void start_up_init_colorbar(void)
{
    ...
    for (i= 0; i < cur_h2d_tbl->colorbar_p1_tbl_size; i++) {
        write_h2d_reg_item(&cur_h2d_tbl->colorbar_p1_tbl[i]);
    }

    context.lcm_drv->initialize();

    for (i= 0; i < cur_h2d_tbl->colorbar_p2_tbl_size; i++) {
        write_h2d_reg_item(&cur_h2d_tbl->colorbar_p2_tbl[i]);
    }
}
#endif

static int disp_fps_set(const h2d_tbl_t *tbl)
{
	if (cur_h2d_tbl == tbl) {
	    disp_warning("already setted.\n");
		return 1;
    }

#if ENABLE_GET_REGS_FROM_PARTION
    if (cur_h2d_tbl == &partion_h2d_tbl) {
        disp_warning("partion h2d table is used.\n");
        return 1;
    }
#endif
...
{
    if (DISPLAY_FUNC_SET00_PHONE_10 == type) {
        return disp_fps_set(&m10_h2d_tbl);
    } else if (DISPLAY_FUNC_SET00_PHONE_U_ULTRA == type) {
        return disp_fps_set(&ocean_note_h2d_tbl);
    } else if (DISPLAY_FUNC_SET00_PHONE_OCEAN == type) {
        return disp_fps_set(&ocean_h2d_tbl);
    } else {
        disp_warning("unknow phone type %d\n", type);
        return -1;
    }
}
...
static void start_up_init(void)
{
    int i = 0;
    int no_hdcp = 0;
    for (i= 0; i < cur_h2d_tbl->startup_p1_tbl_size; i++) {
        write_h2d_reg_item(&cur_h2d_tbl->startup_p1_tbl[i]);
    }

    context.lcm_drv->initialize();

    if (XA0n == pcb_id || XB02 == pcb_id || UNKNOWN == pcb_id) { //skip hdcp settings
        no_hdcp = 1;
    }

    for (i= 0; i < cur_h2d_tbl->startup_p2_tbl_size; i++) {
        if (no_hdcp) {
            if (0x85ec == cur_h2d_tbl->startup_p2_tbl[i].addr
                || 0x8560 == cur_h2d_tbl->startup_p2_tbl[i].addr
                || 0x8563 == cur_h2d_tbl->startup_p2_tbl[i].addr
                || 0x8564 == cur_h2d_tbl->startup_p2_tbl[i].addr) {
                disp_debug("skip hdcp setting: 0x%04x.\n", cur_h2d_tbl->startup_p2_tbl[i].addr);
                continue;
            }
        }

        write_h2d_reg_item(&cur_h2d_tbl->startup_p2_tbl[i]);
    }
}
...
static int get_reg_tbl()
{
    ...
    offset = PARTION_ADDR;
    partion_h2d_tbl.magic_num = *((uint32_t *)offset);
    if (H2D_MAGIC != partion_h2d_tbl.magic_num) {
        return 0;
    }
    offset += 4;
    partion_h2d_tbl.fps = *((float *)offset);

    offset += 4;
    partion_h2d_tbl.startup_p1_tbl_size = *((uint32_t *)offset);
    offset += 4;
    partion_h2d_tbl.startup_p1_tbl = (reg_item *)offset;
    offset += (partion_h2d_tbl.startup_p1_tbl_size*sizeof(reg_item));
    partion_h2d_tbl.startup_p2_tbl_size = *((uint32_t *)offset);
    offset += 4;
    partion_h2d_tbl.startup_p2_tbl = (reg_item *)offset;
    offset += (partion_h2d_tbl.startup_p2_tbl_size*sizeof(reg_item));
    partion_h2d_tbl.colorbar_p1_tbl_size = *((uint32_t *)offset);
    offset += 4;
    partion_h2d_tbl.colorbar_p1_tbl = (reg_item *)offset;
    offset += (partion_h2d_tbl.colorbar_p1_tbl_size*sizeof(reg_item));
    partion_h2d_tbl.colorbar_p2_tbl_size = *((uint32_t *)offset);
    offset += 4;
    partion_h2d_tbl.colorbar_p2_tbl = (reg_item *)offset;


    disp_info("get regs table from fota fps %f %d %d %d %d.\n", partion_h2d_tbl.fps, partion_h2d_tbl.startup_p1_tbl_size,
                                                    partion_h2d_tbl.startup_p2_tbl_size,
                                                    partion_h2d_tbl.colorbar_p1_tbl_size,
                                                    partion_h2d_tbl.colorbar_p2_tbl_size);
