...
#if IOEXP_USE_TASK
osThreadId IOExpTaskHandle;
#endif
#if IOEXP_USE_QUEUE
osMessageQId IoexpMsgQueueHandle;
#endif
...
#if IOEXP_USE_TASK
void StartIOExpTask(void const * argument)
{
	while(1)
	{
	}
...
static void ioexp_gpio_init(void)
{
	...
	if((XB02 == pcb_id)||(XC01 == pcb_id) || (XC02 == pcb_id)){
        ...
        // V_DP_PWM_EN output high
		ioexp_gpio_set_value(IOEXP_REG_DIR, IOEXP_I_V_DP_PWM_EN, IOEXP_GPIO_OUTPUT);
		ioexp_gpio_set_value(IOEXP_REG_OUT, IOEXP_I_V_DP_PWM_EN, IOEXP_GPIO_HIGH);
        // V_PX_PWM_EN output high
		ioexp_gpio_set_value(IOEXP_REG_DIR, IOEXP_II_V_PX_PWM_EN, IOEXP_GPIO_OUTPUT);
		ioexp_gpio_set_value(IOEXP_REG_OUT, IOEXP_II_V_PX_PWM_EN, IOEXP_GPIO_HIGH);
        // V_VDD_1V2_PWM_EN output low
		ioexp_gpio_set_value(IOEXP_REG_DIR, IOEXP_I_V_VDD_1V2_PWM_EN, IOEXP_GPIO_OUTPUT);
		ioexp_gpio_set_value(IOEXP_REG_OUT, IOEXP_I_V_VDD_1V2_PWM_EN, IOEXP_GPIO_LOW);
        // V_VDD_3V3_PWM_EN output low
		ioexp_gpio_set_value(IOEXP_REG_DIR, IOEXP_I_V_VDD_3V3_PWM_EN, IOEXP_GPIO_OUTPUT);
		ioexp_gpio_set_value(IOEXP_REG_OUT, IOEXP_I_V_VDD_3V3_PWM_EN, IOEXP_GPIO_LOW);
	}
}
...
#if IOEXP_USE_QUEUE
static int ioexp_create_queue(void)
{
	osMessageQDef(ioexp_queue_t, 4, Blink_t);
	IoexpMsgQueueHandle = osMessageCreate(osMessageQ(ioexp_queue_t), IOExpTaskHandle);
	if ( IoexpMsgQueueHandle == NULL )
		return -1;
...
#if IOEXP_USE_TASK
	osThreadDef(IOExp_Task, StartIOExpTask, osPriorityNormal, 0, 128);
    IOExpTaskHandle = osThreadCreate(osThread(IOExp_Task), NULL);
#endif
