...
static int fw_fgets(uint8_t * buff, unsigned size, unsigned *offset)
{
...
int  ccgx_upgrade_fw_fw(void)
{
	...
	while (1) {
		...
		if (offset - f_offset >= f_size)
	       ...
	       if (fw_fgets((uint8_t *) line, CYACD_MAX_LINE_LEN, &offset) < 0 ) {
...
int ccgx_upgrade_fw_bl(int fw_idx)
{
	bool stat;
        int retval;
	uint8_t mode, resp;
	...
	if (handle_ccg4 == NULL) {
	  ...
	  ccgx_get_fw_info(fw_idx, &offset, &f_size);
          ...
          ccgx_fw_pr("Upgrade FW start, maybe take many minutes......\n\n");
