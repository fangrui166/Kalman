...
status_t LSM6DSM_Ecompass_SH_init_SI_Matrix(u8_t *SI_matrix)
{
  if(SENSOR_X_0_handle ==( void * )0)
    return MEMS_ERROR;
  return LSM6DSM_ACC_GYRO_SH_init_SI_Matrix(SENSOR_X_0_handle, SI_matrix);
}
status_t LSM6DSM_Ecompass_SH0_Program(u8_t SlvAddr, u8_t Reg, u8_t len)
{
  if(SENSOR_X_0_handle ==( void * )0)
    return MEMS_ERROR;
  return LSM6DSM_ACC_GYRO_SH0_Program(SENSOR_X_0_handle, SlvAddr,Reg,len);
}
status_t LSM6DSM_Ecompass_SH0_ReadMem(u8_t SlvAddr, u8_t Reg, u8_t *Bufp, u8_t len, u8_t stop)
{
  if(SENSOR_X_0_handle ==( void * )0)
    return MEMS_ERROR;
  ...
  if(len != 9)
  {
 	LSM6DSM_ACC_GYRO_W_SensorHUB_Trigger_Sel(SENSOR_X_0_handle, LSM6DSM_ACC_GYRO_START_CONFIG_XL_G_DRDY);
	if(LSM6DSM_ACC_GYRO_SH0_ReadMem(SENSOR_X_0_handle, SlvAddr, Reg, Bufp, len, 0) != MEMS_SUCCESS)
		return MEMS_ERROR;
	
	LSM6DSM_ACC_GYRO_W_DRDY_ON_INT1(SENSOR_X_0_handle, LSM6DSM_ACC_GYRO_DRDY_ON_INT1_ENABLED);
	LSM6DSM_ACC_GYRO_W_SensorHUB_Trigger_Sel(SENSOR_X_0_handle, LSM6DSM_ACC_GYRO_START_CONFIG_EXT_INT2);
	LSM6DSM_Ecompass_Read_rawdata(SENSOR_X_0_handle, SlvAddr);
	
		return MEMS_SUCCESS;
   }else
   {
	if( !LSM6DSM_ACC_GYRO_ReadMem(SENSOR_X_0_handle, LSM6DSM_ACC_GYRO_SENSORHUB1_REG, Bufp, len))
		return MEMS_ERROR;
   }

   return MEMS_SUCCESS;
}
status_t LSM6DSM_Ecompass_SH0_WriteByte(u8_t SlvAddr, u8_t Reg, u8_t Bufp)
{
  if(SENSOR_X_0_handle ==( void * )0)
    return MEMS_ERROR; 
  LSM6DSM_ACC_GYRO_W_SensorHUB_Trigger_Sel(SENSOR_X_0_handle, LSM6DSM_ACC_GYRO_START_CONFIG_XL_G_DRDY);
  LSM6DSM_ACC_GYRO_SH0_WriteByte(SENSOR_X_0_handle, SlvAddr,Reg, Bufp);
...
status_t LSM6DSM_Ecompass_Read_rawdata(void *handle, u8_t SlvAddr)
{

  u8_t Slv0AddrBufp = 0x10;
  u8_t Slv0LenBufp = 0x10 | 0x7;//SLAVE0_CONFIG  Aux_sens_on[1:0]  two sensors
  u8_t Slv1AddrBufp = 0x17;
  u8_t Slv1LenBufp = 0x2;
  ...
  /* Write remote device I2C subaddress */
  if( !LSM6DSM_ACC_GYRO_WriteMem(handle, LSM6DSM_ACC_GYRO_SLV0_SUBADD, &Slv0AddrBufp, 1))
    return MEMS_ERROR;

  /* Write number of bytes to read */
  if( !LSM6DSM_ACC_GYRO_WriteMem(handle, LSM6DSM_ACC_GYRO_SLAVE0_CONFIG, &Slv0LenBufp, 1))
    return MEMS_ERROR;

  if( !LSM6DSM_ACC_GYRO_WriteMem(handle, LSM6DSM_ACC_GYRO_SLV1_ADD, &SlvAddr, 1))
    return MEMS_ERROR;

  /* Write remote device I2C subaddress */
  if( !LSM6DSM_ACC_GYRO_WriteMem(handle, LSM6DSM_ACC_GYRO_SLV1_SUBADD, &Slv1AddrBufp, 1))
    return MEMS_ERROR;

  /* Write number of bytes to read */
  if( !LSM6DSM_ACC_GYRO_WriteMem(handle, LSM6DSM_ACC_GYRO_SLAVE1_CONFIG, &Slv1LenBufp, 1))
    return MEMS_ERROR;
  /* Close Embedded Function Register page*/
  if( !LSM6DSM_ACC_GYRO_W_EmbeddedAccess(handle, LSM6DSM_ACC_GYRO_EMBEDDED_ACCESS_DISABLED))
    return MEMS_ERROR;
