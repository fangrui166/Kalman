...
static SemaphoreHandle_t	xSpi2SemaphoreMutex;
...
{
	xSpi2SemaphoreMutex = NULL;
	xSpi2SemaphoreMutex = osMutexCreate(NULL);
	if(xSpi2SemaphoreMutex == NULL){
		gsensor_err("Create SPI2 mutex fail\r\n");
		return -1;
	}
	return 0;
}
...
int8_ts bmi160_spi_bus_read(us32 dev_addr, us8 reg_addr, us8 *reg_data, us8 cnt)
{
	...
	if(osMutexWait(xSpi2SemaphoreMutex, portMAX_DELAY) == osOK){
		status = HAL_OK;
		cnt++;
		...
		if(status != HAL_OK)
		{
			...
			SPI2_Error();
			osMutexRelease(xSpi2SemaphoreMutex);
			return -1;
		}
