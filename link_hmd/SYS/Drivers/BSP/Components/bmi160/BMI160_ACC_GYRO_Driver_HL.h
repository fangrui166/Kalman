...
#define GSENSOR_HLOG_ENABLE
#ifdef GSENSOR_HLOG_ENABLE
#define gsensor_emerg(fmt, ...) \
	hlog_printf(HLOG_LVL_EMERG, HLOG_TAG_SENSOR,"[G+G] " fmt, ##__VA_ARGS__)
#define gsensor_err(fmt, ...) \
	hlog_printf(HLOG_LVL_ERR, HLOG_TAG_SENSOR,"[G+G] " fmt, ##__VA_ARGS__)
#define gsensor_warning(fmt, ...) \
	hlog_printf(HLOG_LVL_WARNING, HLOG_TAG_SENSOR,"[G+G] " fmt, ##__VA_ARGS__)
#define gsensor_info(fmt, ...) \
	hlog_printf(HLOG_LVL_INFO, HLOG_TAG_SENSOR,"[G+G] " fmt, ##__VA_ARGS__)
#define gsensor_debug(fmt, ...) \
	hlog_printf(HLOG_LVL_DEBUG, HLOG_TAG_SENSOR,"[G+G] " fmt, ##__VA_ARGS__)
#else /* GSENSOR_HLOG_ENABLE */
#define gsensor_emerg(fmt, ...) \
	printf("<0>[SENSOR] [G+G] " fmt, ##__VA_ARGS__)
#define gsensor_err(fmt, ...) \
	printf("<1>[SENSOR] [G+G] " fmt, ##__VA_ARGS__)
#define gsensor_warning(fmt, ...) \
	printf("<2>[SENSOR] [G+G] " fmt, ##__VA_ARGS__)
#define gsensor_info(fmt, ...) \
	printf("<3>[SENSOR] [G+G] " fmt, ##__VA_ARGS__)
#define gsensor_debug(fmt, ...) \
	printf("<4>[SENSOR] [G+G] " fmt, ##__VA_ARGS__)
#endif /* GSENSOR_HLOG_ENABLE */
#endif
#endif
