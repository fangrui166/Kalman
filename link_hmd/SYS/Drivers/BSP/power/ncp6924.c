...
int8_t ncp6924_reg_write(uint8_t reg, uint8_t value)
{
    ...
    if(ret != I2C_OK)
    {
		pm_err("%s ***write reg(0x%2x) value(0x%2x) *** fail\r\n", __func__, reg, value);
        return -1;
    }
	else
	{
...
static int8_t ncp6924_reg_write_enable_bit(uint8_t index, uint8_t value)
{
	...
	if(ret != I2C_OK)
	{
        pm_err("%s ***read reg(0x%2x) value(0x%2x) *** fail\r\n", __func__, tx_buf[0], rx_buf);
		return -1;
	}
...
static int NCP6924_Deinit(PmicDrvTypeDef *handle)
{
    ...
    if(XA0n == pcb_id){
        ...
        HAL_GPIO_WritePin(GPIO_XB_PORT_PMIC_EN, GPIO_XB_PMIC_EN, GPIO_PIN_RESET);
    }
    is_ncp6924_init = 0;
