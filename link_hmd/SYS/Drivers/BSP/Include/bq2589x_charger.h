#ifndef __BQ25896_CHARGER_H__
#define __BQ25896_CHARGER_H__
...
//#define BQ2589X_CONV_START           1	/*sample code define BQ2589X_CONV_START = 0*/
...
#define BQ2589X_EN_STAT_PIN_MASK       0x40
#define BQ2589X_EN_STAT_PIN_SHIFT      6
#define BQ2589X_CHG_STAT_PIN_DISABLE   1
#define BQ2589X_CHG_STAT_PIN_ENABLE   0
...
#define BQ25896_I2C_ADDRESS		0x6B << 1		/*whether should be 0x6B*/
#define BQ2589x_I2C_TIMEOUT_MAX 	1000

#define BQ2589x_TIMER_TIMEOUT		(configTICK_RATE_HZ * 180)
...
typedef struct{
    ...
    int (* Set_InputCurrentLimit )(PmicDrvTypeDef*, uint32_t);
    int (* Set_Charger_EnableDisable )(PmicDrvTypeDef*, bool);
    int (* Get_vBusVoltage )(PmicDrvTypeDef*, uint32_t *);
    int (* Get_chargingPara )(PmicDrvTypeDef*, chargerPara *);
    int (* Get_pgState)(PmicDrvTypeDef*, uint8_t *);
    int (* Set_ChargeVoltageLimit )(PmicDrvTypeDef*, uint32_t);
    int (* Set_ChargeCurrentLimit )(PmicDrvTypeDef*, uint32_t);
    int (* PowerOff )(PmicDrvTypeDef*);
    int (* PowerReset )(PmicDrvTypeDef*);
    int (* Set_HizMode)(PmicDrvTypeDef*, bool);
}CHARGER_DRV_t;

extern CHARGER_DRV_t BQ25896_Drv;

typedef struct chg_status {
	float batt_temp;
	uint8_t online;
}CHG_STAT;
...
enum chg_fault_event_type {
	CHG_FALUT_EVENT_UNKOWN,
	CHG_FALUT_EVENT_NORMAL,
	CHG_FALUT_EVENT_INPUT,
	CHG_FALUT_EVENT_THERMAL,
	CHG_FALUT_EVENT_TIMER,
};

#define CHG_HLOG_ENABLE		1

#ifdef CHG_HLOG_ENABLE
#define chg_emerg(fmt, ...) \
	hlog_printf(HLOG_LVL_EMERG, HLOG_TAG_CHG, fmt, ##__VA_ARGS__)
#define chg_err(fmt, ...) \
	hlog_printf(HLOG_LVL_ERR, HLOG_TAG_CHG, fmt, ##__VA_ARGS__)
#define chg_warning(fmt, ...) \
...
#else /* CHG_HLOG_ENABLE */
#define chg_emerg(fmt, ...) \
	printf("[CHG][EMR] :" fmt, ##__VA_ARGS__)
#define chg_err(fmt, ...) \
	printf("[CHG][ERR] :" fmt, ##__VA_ARGS__)
#define chg_warning(fmt, ...) \
	printf("[CHG][WARN]:" fmt, ##__VA_ARGS__)
#define chg_info(fmt, ...) \
	printf("[CHG][INFO]:" fmt, ##__VA_ARGS__)
#define chg_debug(fmt, ...) \
	printf("[CHG][DBG] :" fmt, ##__VA_ARGS__)
#endif /* CHG_HLOG_ENABLE */
