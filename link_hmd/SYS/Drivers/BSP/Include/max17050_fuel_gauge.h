...
typedef struct{
	...
	int (* Get_Property)(PmicDrvTypeDef *,
...
#define FG_HLOG_ENABLE	1

#ifdef FG_HLOG_ENABLE
#define fg_emerg(fmt, ...) \
	hlog_printf(HLOG_LVL_EMERG, HLOG_TAG_FG, fmt, ##__VA_ARGS__)
#define fg_err(fmt, ...) \
	hlog_printf(HLOG_LVL_ERR, HLOG_TAG_FG, fmt, ##__VA_ARGS__)
#define fg_warning(fmt, ...) \
	hlog_printf(HLOG_LVL_WARNING, HLOG_TAG_FG, fmt, ##__VA_ARGS__)
#define fg_info(fmt, ...) \
	hlog_printf(HLOG_LVL_INFO, HLOG_TAG_FG, fmt, ##__VA_ARGS__)
#define fg_debug(fmt, ...) \
	hlog_printf(HLOG_LVL_DEBUG, HLOG_TAG_FG, fmt, ##__VA_ARGS__)
#else /* FG_HLOG_ENABLE */
#define fg_emerg(fmt, ...) \
