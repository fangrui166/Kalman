#include <stdio.h>
#include <math.h>
#include "Fusion/So3Util.h"
#include "Fusion/OrientationEKF.h"
#include "hlog_api.h"
...
void OrientationEKF_reset(OrientationEKF* this_)
{
    ...
    this_->lastAngleAcc = &this_->Instance_lastAngleAcc;
    new_Vector3d(this_->lastAngleAcc);
...
float* plugin_getPredictedGLMatrix(float secondsAfterLastGyroEvent,
                                   OrientationEKF* this_)
{
    return glMatrixFromSo3(this_->so3SensorFromWorld, this_);
}
...
{
#ifdef DEBUG
    hlog_printf(HLOG_LVL_INFO, HLOG_TAG_FUSION, "[processGyro] init gyro x=%f, y=%f, z=%f, sensorTimeStamp=%lld, sensorTimeStampGyro=%lld\n",
            gyro->x, gyro->y, gyro->z, sensorTimeStamp, this_->sensorTimeStampGyro);
#endif /* DEBUG */
...
#ifdef DEBUG
        hlog_printf(HLOG_LVL_INFO, HLOG_TAG_FUSION, "[processGyro] dT=%f, sensorTimeStamp - sensorTimeStampGyro=%lld\n",
                dT, sensorTimeStamp - this_->sensorTimeStampGyro);
#endif /* DEBUG */

        // + get angleAcc
        this_->lastAngleAcc->x = (gyro->x - this_->lastGyro->x) / dT;
        this_->lastAngleAcc->y = (gyro->y - this_->lastGyro->y) / dT;
        this_->lastAngleAcc->z = (gyro->z - this_->lastGyro->z) / dT;
        // - get angleAcc
...
#ifdef DEBUG
        for (int i = 0; i < 9; i++) {
            hlog_printf(HLOG_LVL_INFO, HLOG_TAG_FUSION, "[processGyro] mult processGyroTempM1.m[%d]=%f\n",
                    i, this_->processGyroTempM1->m[i]);
        }
#endif /* DEBUG */
...
#ifdef DEBUG
        for (int i = 0; i < 9; i++) {
            hlog_printf(HLOG_LVL_INFO, HLOG_TAG_FUSION, "[processGyro] plusEquals mP.m[%d]=%f\n",
                    i, this_->mP->m[i]);
        }
#endif /* DEBUG */
...
#ifdef DEBUG
    hlog_printf(HLOG_LVL_INFO, HLOG_TAG_FUSION, "[processGyro] lastGyro x=%f, y=%f, z=%f\n",
            this_->lastGyro->x, this_->lastGyro->y, this_->lastGyro->z);
#endif /* DEBUG */
...
#ifdef DEBUG
    hlog_printf(HLOG_LVL_INFO, HLOG_TAG_FUSION, "[updateAccelCovariance] accelNoiseSigma=%f, accelNoiseSigma * accelNoiseSigma=%f\n",
            accelNoiseSigma, accelNoiseSigma * accelNoiseSigma);
#endif /* DEBUG */
...
#ifdef DEBUG
    hlog_printf(HLOG_LVL_INFO, HLOG_TAG_FUSION, "[processAcc] init acc x=%f, y=%f, z=%f, sensorTimeStamp=%lld\n",
...
#ifdef DEBUG
        for (int i = 0; i < 9; ++i) {
            hlog_printf(HLOG_LVL_INFO, HLOG_TAG_FUSION, "[processAcc] so3SensorFromWorld mult [%d]=%f\n",
...
#ifdef DEBUG
        for (int i = 0; i < 9; ++i) {
            hlog_printf(HLOG_LVL_INFO, HLOG_TAG_FUSION, "[processAcc] So3Util_sO3FromTwoVec so3SensorFromWorld [1] [%d]=%f\n",
                    i, this_->so3SensorFromWorld->m[i]);
        }
#endif /* DEBUG */
...
#ifdef DEBUG
        for (int i = 0; i < 9; ++i) {
            hlog_printf(HLOG_LVL_INFO, HLOG_TAG_FUSION, "[processAcc] So3Util_sO3FromTwoVec so3SensorFromWorld [2] [%d]=%f\n",
                    i, this_->so3SensorFromWorld->m[i]);
        }
#endif /* DEBUG */
...
void processMag(float mag[], long long sensorTimeStamp, OrientationEKF* this_)
{
...
#ifdef DEBUG
    hlog_printf(HLOG_LVL_INFO, HLOG_TAG_FUSION, "[processMag] init Mag x=%f, y=%f, z=%f, sensorTimeStamp=%lld\n",
    ...
    if (this_->alignedToGravity) {
        ...
        if (this_->alignedToNorth) {
...
#ifdef DEBUG
            for (int i = 0; i < 9; i++) {
                hlog_printf(HLOG_LVL_INFO, HLOG_TAG_FUSION, "[processMag] so3SensorFromWorld[1] this.so3SensorFromWorld.m[%d]=%f\n",
                        i, this_->so3SensorFromWorld->m[i]);
            }
#endif /* DEBUG */
            ...
            for (int i = 0; i < 9; i++) {
                hlog_printf(HLOG_LVL_INFO, HLOG_TAG_FUSION, "[processMag] so3SensorFromWorld[2] this.so3SensorFromWorld.m[%d]=%f\n",
                        i, this_->so3SensorFromWorld->m[i]);
            }
#endif /* DEBUG */
...
#ifdef DEBUG
    for (int i = 0; i < 9; i++) {
        hlog_printf(HLOG_LVL_INFO, HLOG_TAG_FUSION, "[updateCovariancesAfterMotion] mP.m[%d]=%f\n",
                i, this_->mP->m[i]);
    }
#endif /* DEBUG */
