...
void setRotateEuler3x3M(float* rm, int rmOffset, float x, float y, float z)
{
    float cx;
    float sx;
    float cy;
...
// Euler formula is wrong on anroid java source code, correct in #if 1
#if 1
    rm[rmOffset + 3] = sxsy * cz + cx * sz;
    rm[rmOffset + 4] = -sxsy * sz + cx * cz;
#else
    rm[rmOffset + 3] = cxsy * cz + cx * sz;
    rm[rmOffset + 4] = -cxsy * sz + cx * cz;
#endif
    rm[rmOffset + 5] = -sx * cy;
    //rm[rmOffset + 7] = 0.0f;
...
// Euler formula is wrong on anroid java source code, correct in #if 1
#if 1
    rm[rmOffset + 6] = -cxsy * cz + sx * sz;
    rm[rmOffset + 7] = cxsy * sz + sx * cz;
#else
    rm[rmOffset + 6] = -sxsy * cz + sx * sz;
    rm[rmOffset + 7] = sxsy * sz + sx * cz;
#endif
    rm[rmOffset + 8] = cx * cy;
}
#if 0
