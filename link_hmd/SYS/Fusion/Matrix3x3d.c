...
#else
    arm_matrix_instance_f32 pSrcA, pSrcB, pDst;
    arm_mat_init_f32(&pSrcA, 3, 3, a->m);
    arm_mat_init_f32(&pSrcB, 3, 3, b->m);
    arm_mat_init_f32(&pDst, 3, 3, result->m);
...
void Matrix3x3d_mult_V(Matrix3x3d* a, Vector3d* v, Vector3d* result)
{
    ...
    arm_matrix_instance_f32 pSrcA, pSrcB, pDst;
    arm_mat_init_f32(&pSrcA, 3, 3, a->m);
    arm_mat_init_f32(&pSrcB, 3, 1, (float*)v);
    arm_mat_init_f32(&pDst, 3, 1, (float*)result);
    arm_mat_mult_f32(&pSrcA, &pSrcB, &pDst);
#endif /* NO_DSP */
}
...
void Matrix3x3d_scale(float s, Matrix3x3d* this_)
{
    ...
    this_->m[8] *= s;
#else
    arm_matrix_instance_f32 pSrcA, pDst;
    arm_mat_init_f32(&pSrcA, 3, 3, this_->m);
    arm_mat_init_f32(&pDst, 3, 3, this_->m);
    arm_mat_scale_f32(&pSrcA, s, &pDst);
#endif /* NO_DSP */
}
...
bool Matrix3x3d_invert(Matrix3x3d* result, Matrix3x3d* this_)
{
    ...
    arm_mat_init_f32(&pSrcA, 3, 3, this_->m);
    arm_mat_init_f32(&pDst, 3, 3, result->m);
    arm_mat_inverse_f32(&pSrcA, &pDst);
    return true;
#endif /* NO_DSP */
}
