#include <stdio.h>
#include "Fusion/GyroscopeBiasEstimator.h"
#include "hlog_api.h"
...
{
    /*TODO*/
    if (Vector3d_length(gyro) < 0.02 /*0.3499999940395355*/) {
        float updateWeight = max(0.0,
                                 1.0 - Vector3d_length(gyro) / 0.02 /*0.3499999940395355*/);
...
#ifdef DEBUG
        hlog_printf(HLOG_LVL_INFO, HLOG_TAG_FUSION, "[processGyroscope] smoothedGyroDiff length=%f \n", Vector3d_length(this_->smoothedGyroDiff));
#endif /* DEBUG */
...
#ifdef DEBUG
    hlog_printf(HLOG_LVL_INFO, HLOG_TAG_FUSION, "[processAccelerometer] smoothedAccelDiff x=%f, y=%f, z=%f\n",
            this_->smoothedAccelDiff->x, this_->smoothedAccelDiff->y, this_->smoothedAccelDiff->z);
#endif /* DEBUG */
...
#ifdef DEBUG
    hlog_printf(HLOG_LVL_INFO, HLOG_TAG_FUSION, "[getGyroBias] GyroBias x=%f, y=%f, z=%f\n", result->x, result->y, result->z);
#endif /* DEBUG */
}
...
{
#ifdef DEBUG
    hlog_printf(HLOG_LVL_INFO, HLOG_TAG_FUSION, "[appendFrame] isStatic=%d\n", isStatic);
#endif /* DEBUG */
